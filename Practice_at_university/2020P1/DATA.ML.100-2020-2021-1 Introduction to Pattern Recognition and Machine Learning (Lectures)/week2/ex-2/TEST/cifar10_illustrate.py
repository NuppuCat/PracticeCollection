...
XW = datadicW["data"]
YW = datadicW["labels"]
XW = XW.reshape(60000, 3, 32, 32).transpose(0,2,3,1).astype("uint8")
YW = np.array(YW)



#set X Y as train set
X = datadict["data"]
Y = datadict["labels"]
...
def cifar10_classifier_random(x):
    a = np.random.randint(0,len(labeldict),len(x))
    return a
def cifar10_classifier_1nn(x,trdata,trlabels):
    ...
    for j in range(0,len(x)):
        ...
        bestLabel[j] = trlabels[bestInd[j]]
    
    return bestLabel
def plot_picture_with_label(ind,lab):
...
testIdx = np.random.randint(0,len(XW),10)

# test random classifier
y = cifar10_classifier_random(XW[testIdx])

plot_picture_with_label(testIdx,y)

print(f"{y} and {YW[testIdx]}")

accRandom = class_acc(y,YW[testIdx])

print(f"the accurate rate of random classifier in this time is {accRandom}%")


# test 1nn classifier
label = cifar10_classifier_1nn(XW[testIdx],X,Y)
plot_picture_with_label(testIdx,label)
print(f"{label} and {YW[testIdx]}")
accNN1 = class_acc(label,YW[testIdx])
print(f"the accurate rate of NN1 classifier in this time is {accNN1}%")

# test 1nn classifier with x in train set
testIdxt = np.random.randint(0,len(X),10)
labelt = cifar10_classifier_1nn(X[testIdxt],X,Y)
plot_picture_with_label(testIdxt,labelt)
print(f"{labelt} and {Y[testIdxt]}")
accNN1t = class_acc(labelt,Y[testIdxt])
