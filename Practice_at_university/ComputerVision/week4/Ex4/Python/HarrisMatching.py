...
for i1 in range(npts1):
    for i2 in range(npts2):
        top = np.sum((patches1[:,:,i1]-np.mean(patches1[:,:,i1]))*(patches2[:,:,i2]-np.mean(patches2[:,:,i2])))
        bot = np.sqrt(np.sum((patches1[:,:,i1]-np.mean(patches1[:,:,i1]))**2)*np.sum((patches2[:,:,i2]-np.mean(patches2[:,:,i2]))**2))
        distmat[i1, i2] =top/bot
cc1 = np.amax(distmat, axis=1)
idc1 = np.argmax(distmat, axis=1)
cc2 = np.amax(distmat, axis=0)
idc2 = np.argmax(distmat, axis=0)

pairs = []
for k in range(npts1):
    if k == idc2[idc1[k]]:
        pairs.append(np.array([k, idc1[k], cc1[k]]))
pairs = np.array(pairs)

# We sort the mutually nearest neighbors based on the NCC
sorted_ncc = -np.sort(-1*pairs[:,2], axis=0)
id_ncc = np.argsort(-1*pairs[:,2], axis=0)
##-your-code-ends-here-##
#%%
