...
import sys
import pathlib
sys.path.append('D:/GRAM/MasterProgramme/Tampere/ComputerVision/week1/Exercise1/Python')
...
im = imread('D:/GRAM/MasterProgramme/Tampere/ComputerVision/week1/Exercise1/Python/einsteinpic.jpg') / 255.
...
simplee = np.array([[1,2,1],[2,4,2],[1, 2 ,1]])
u,e,v1 =svd(simplee)
h = np.sqrt(e[0])*u[:,0]
v = np.sqrt(e[0])*v1[0]
print(np.outer(h,v))
...
# h=h[np.newaxis, :]
gflt_imns_s = conv1(imns, v, axis = 1, mode='reflect')
gflt_imns_s = conv1(gflt_imns_s, h,axis = 0, mode='reflect')
gflt_imng_s = conv1(imng, h,axis = 1, mode='reflect')
gflt_imng_s = conv1(gflt_imng_s, v,axis = 0, mode='reflect')
print(np.allclose(gflt_imns,gflt_imns_s))
print(np.allclose(gflt_imng,gflt_imng_s))
##--your-code-ends-here--##
#%%
