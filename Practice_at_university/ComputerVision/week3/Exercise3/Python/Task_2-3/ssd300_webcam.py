...
while (cam.isOpened()):
    ...
    ret,img =  cam.read()  # dummy image for testing

    ##-your-code-starts-here-##
    image = imresize(img,(img_height, img_width),preserve_range=True)
    image = np.array([image])
    detections=model.predict(image)
    
    for i in range(detections.shape[2]):
        confidence = detections[0,  i, 1] #Confidence of prediction 
        if confidence >confidence_threshold: # Filter prediction 
            class_id = int(detections[0,  i, 0]) # Class label
            # class_id, confidence, xmin, ymin, xmax, ymax
            # Object location 
            xmin = int(detections[0,  i, 2] * img.shape[1]/ img_width)
            ymin = int(detections[0,  i, 3] *  img.shape[0]/ img_height)
            xmax   = int(detections[0,  i, 4] *  img.shape[1]/ img_width)
            ymax   = int(detections[0,  i, 5] * img.shape[0]/ img_height)
            img=cv2.rectangle(img, (xmin, ymin), (xmax,ymax), (0, 255, 0), 2)
            img= cv2.putText(img, classes[class_id], (xmin, ymin),  cv2.FONT_HERSHEY_SIMPLEX,
